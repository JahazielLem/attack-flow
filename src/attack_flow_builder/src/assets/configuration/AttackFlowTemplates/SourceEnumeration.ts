...
import SpartaEnums from "./MitreSparta";
import SpaceShieldEnums from "./SpaceShield";
...
export interface SourceRelationships {
    techniqueSubtechniques?: { techniqueId: string, subtechniqueId: string }[];
...
export interface SourceData {
    ...
    subtechniques?: Record<string, SourceObject>;
...
function toRelationshipRows(relationships: SourceRelationships): string[][] {
    const isSubtechnique = (id: string) => sourceData.techniques[id]?.type === "subtechnique";
    return [
        ...(relationships.techniqueSubtechniques ?? []).map(
            rel => ["technique", rel.techniqueId, "subtechnique", rel.subtechniqueId]
        ),
        ...relationships.tacticTechniques.flatMap(rel => [
            ["tactic", rel.tacticId, "technique", rel.techniqueId],
            ...(isSubtechnique(rel.techniqueId)
                ? [["tactic", rel.tacticId, "subtechnique", rel.techniqueId]] : [])
        ]),
...
export const sourceData: SourceData = sources.reduce<SourceData>((acc, src) => {
    ...
    mergeObjects(acc.subtechniques!, src.subtechniques ?? {});
    acc.relationships.techniqueSubtechniques!.push(...(src.relationships.techniqueSubtechniques ?? []));
...
const enums: SourceEnums = {
    ...
    subtechniques: toOptionList(sourceData.subtechniques ?? {}),
