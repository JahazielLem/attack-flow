...
describe("AttackFlowRecommender", () => {
    ...
    it.each(["SPA", "SSH"])("recommends countermeasures for the selected %s subtechnique", async framework => {
        const mappings = sourceData.relationships.techniqueMitigations;
        const mitigationIds = (id: string) => mappings.filter(rel => rel.techniqueId === id)
            .map(rel => rel.mitigationId).sort();
        const relation = sourceData.relationships.techniqueSubtechniques!.find(rel =>
            sourceData.techniques[rel.subtechniqueId].label.startsWith(`[${framework}]`)
            && mitigationIds(rel.subtechniqueId).length > 0
            && JSON.stringify(mitigationIds(rel.subtechniqueId)) !== JSON.stringify(mitigationIds(rel.techniqueId))
        );
        expect(relation).toBeDefined();
        const file = await createAttackFlowFile();
        selectFrameworks(file, [framework]);
        const latch = createDanglingTargetLatch(file, relation!.techniqueId);
        const action = (latch.parent as LineView).sourceObject!;
        action.properties.get("ttp", TTPTupleProperty)!.get("subtechnique", StringProperty)!
            .setValue(relation!.subtechniqueId);

        const recommendations = (await startRecommender(file, latch).getRecommendations()).items;

        expect(childRecommendations(recommendations, "mitigation").map(row => row.id).sort())
            .toEqual(mitigationIds(relation!.subtechniqueId));
